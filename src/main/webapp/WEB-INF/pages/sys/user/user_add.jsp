  ...
  $(function(){
	...
	$("#form-user-add").validate({
		...
		submitHandler:function(form){
			$.post('<%=basePath%>user/saveUser.do',$(form).serialize(),function(data) {
	...
	<form action="" method="post" class="form form-horizontal" id="form-user-add">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9 skin-minimal">
				<div class="radio-box">
					<input name="gender" type="radio" id="male" value="0" <c:if test="${op == 'add'}">checked="checked"</c:if> <c:if test="${op == 'modify' && user.gender == 0}">checked="checked"</c:if>>
					<label for="sex-1">男</label>
				</div>
				<div class="radio-box">
					<input name="gender" type="radio" id="female" value="1" <c:if test="${op == 'modify' && user.gender == 1}">checked="checked"</c:if>>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" placeholder="@" value="${user.email}" name="address" id="address">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3">备注：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<textarea name="remark" id="userRemark" cols="" rows="" class="textarea"  placeholder="说点什么..." onkeyup="checkLen(this)" >${user.remark }</textarea>
		...
		<c:if test="${op == 'modify'}">
		      <div class="row cl">
			       ...
			       <div class="formControls col-xs-8 col-sm-9 skin-minimal">
				        <div class="radio-box">
					         <input name="status" type="radio" id="userStatus_0" value="0" <c:if test="${op == 'add'}">checked="checked"</c:if> <c:if test="${op == 'modify' && user.status == 0}">checked="checked"</c:if>>
					         <label for="userStatus-1">禁用</label>
				        </div>
				        <div class="radio-box">
					         <input name="status" type="radio" id="userStatus_1" value="1" <c:if test="${op == 'modify' && user.status == 1}">checked="checked"</c:if>>
