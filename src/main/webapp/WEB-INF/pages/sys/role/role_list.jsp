...
<div class="page-container">
    <div class="cl pd-5 bg-1 bk-gray"><span class="l"> <a href="javascript:;" onclick="batch_delete_role()"
                                                          class="btn btn-danger radius"><i
            class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a class="btn btn-primary radius" href="javascript:;"
                                                          onclick="role_add('添加图片信息','<%=basePath %>role/addRole.do','','310')"><i
            class="Hui-iconfont">&#xe600;</i> 添加角色</a> </span> <span class="r">共有数据：<strong>${roleList.size()}</strong> 条</span>
    </div>
        ...
        <c:forEach items="${roleList}" var="role">
            <tr class="text-c">
                ...
                <td class="f-14"><a title="编辑" href="javascript:;"
                                    onclick="admin_role_edit('角色编辑','<%=basePath %>role/editRole.do','${role.id }','','310')"
                                    style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
                    <a title="删除" href="javascript:;" onclick="admin_role_del(this,'${role.id}')" class="ml-5"
                       style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
                    <a title="授权" href="javascript:;"
                       onclick="role_menu('为角色赋权限','<%=basePath %>role/roleMenu.do','${role.id}',400,400)" class="ml-5"
...
<script type="text/javascript">
    ...
    function admin_role_del(obj, id) {
        if (id == 1 || id == 2 || id == 3) {
            layer.msg("默认角色，无法删除！", {icon: 5, time: 2000});
        } else {
            layer.confirm('确认要删除吗？', function (index) {
                $.getJSON('<%=basePath%>role/roledel', {"id": id}, function (data) {
    ...
    function batch_delete_role() {
        ...
        if (length != 0) {
            for (var i = 0; i < length; i++) {
                var selectedId = items[i].value;
                if (selectedId == 1 || selectedId == 2 || selectedId == 3) {
                    layer.msg("所选角色中包含默认角色，无法删除！", {icon: 5, time: 2000});
                    return;
                }
                ids += items[i].value;
                ids += ",";
