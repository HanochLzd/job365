/**
 * Created by chaoxing on 2017/1/5.
 */
$(document).ready(function(){
    ...
    $.ajax({
        type: "get",
        url: "http://shxbe.chaoxing.com/front/localNews/newsList?nodeid=3256&curPage=1&pageSize=10",
        dataType: "json",
        success: function(data){
            if (data.list!=undefined&&data.list!=null&data.list!=''){
                ...
                for (var i=0;i<bnrData.length;i++){
                    bnr.append(' <a class="d1" style="background:url('+url1+bnrData[i].preview+') no-repeat center;background-size: cover;"><div class="des"><span>'+bnrData[i].title+'</span></div></a>');
                    $("ul",d2).append('<li></li>');
                }
                bnr.append(d2);
    ...
    $.ajax({
        type: "get",
        url: "http://shxbe.chaoxing.com/front/localNews/newsList?nodeid=79345&curPage=1&pageSize=5",
        dataType: "json",
        success: function(data){
            if (data.list!=undefined&&data.list!=null&data.list!=''){
    ...
    function linkType(nodeid){
        $.ajax({
            type: "get",
            url: "http://192.168.98.101/news?nodeid="+nodeid+"&curPage=1&pageSize=18",
            dataType: "json",
            success: function(data){
                if (data.list!=undefined&&data.list!=null&data.list!=''){
    ...
    function news(nodeid){
        $.ajax({
            type: "get",
            url: "http://192.168.98.101/news?nodeid="+nodeid+"&curPage=1&pageSize=9",
            dataType: "json",
            success: function(data){
                if (data.list!=undefined&&data.list!=null&data.list!=''){
    ...
    setTimeout(function(){ news(4965);},700);

    /*交流群*/
    $.ajax({
        type: "get",
        url: "http://192.168.98.101/news?nodeid=764&curPage=1&pageSize=4",
        dataType: "json",
        success: function(data){
            if(data.list!=undefined&&data.list!=null&data.list!=''){
